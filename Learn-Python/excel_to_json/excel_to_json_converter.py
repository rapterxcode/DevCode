...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        """
        ฟังก์ชันเริ่มต้นการสร้างส่วนติดต่อผู้ใช้ (UI)
        - สร้างหน้าต่างหลักและกำหนดขนาด
        - สร้างส่วนประกอบต่างๆ เช่น ปุ่ม, ตาราง, กล่องข้อความ
        - กำหนดสไตล์และการจัดวาง
        """
        self.setWindowTitle('Excel/JSON Converter & Editor')
        self.setGeometry(100, 100, 1200, 1000)
        self.setMinimumSize(800, 600)  # กำหนดขนาดหน้าต่างขั้นต่ำ

        # สร้าง widget หลักและ layout
        ...
        # ส่วนบน - การเลือกไฟล์และแถบความคืบหน้า
        top_section = QWidget()
        ...
        # ส่วนล่าง - แบ่งเป็นตารางและตัวแก้ไข JSON
        bottom_section = QWidget()
        bottom_layout = QHBoxLayout(bottom_section)
        bottom_layout.setSpacing(10)

        # ด้านซ้าย - แสดงสถานะและปุ่มจัดการ
        ...
        status_layout.addStretch()  # เพิ่มพื้นที่ว่างเพื่อดันปุ่มขึ้นด้านบน
        bottom_layout.addWidget(status_container, stretch=1)

        # ด้านขวา - ตัวแก้ไข JSON
        ...
        self.table_widget.setSortingEnabled(True)  # เปิดใช้งานการเรียงลำดับ
        ...
        """)
        # เริ่มต้นตารางด้วยคอลัมน์เริ่มต้น
        self.table_widget.setColumnCount(3)
        self.table_widget.setRowCount(1)
        self.table_widget.setHorizontalHeaderLabels(['Column 1', 'Column 2', 'Column 3'])
        table_layout.addWidget(self.table_widget)
        content_layout.addWidget(table_section, stretch=2)  # ให้พื้นที่ตารางมากกว่า
    ...
    def select_file(self):
        ...
        - รองรับไฟล์ Excel และ JSON
        - อัปเดตสถานะและปุ่มต่างๆ ตามไฟล์ที่เลือก
        """
    ...
    def start_conversion(self):
        """
        ฟังก์ชันเริ่มการแปลงไฟล์ Excel เป็น JSON
        - สร้าง worker สำหรับแปลงข้อมูล
        - ใช้ฟังก์ชัน check_and_convert_json_value ในการแปลงข้อมูล
        - แสดงความคืบหน้าและสถานะการแปลง
        """
        ...
        # สร้าง worker และส่งฟังก์ชัน check_and_convert_json_value ไปให้ใช้
    ...
    def update_progress(self, value):
        ...
        รับค่า value เป็นเปอร์เซ็นต์ความคืบหน้า
        """
    ...
    def update_status(self, message):
        ...
        รับค่า message เป็นข้อความที่ต้องการแสดง
        """
        self.status_display.append(message)
        QApplication.processEvents()  # บังคับให้อัปเดต GUI ทันที

    def conversion_finished(self, result):
        """
        ฟังก์ชันจัดการเมื่อการแปลงเสร็จสิ้น
        - แสดงผลลัพธ์การแปลง
    ...
    def json_to_excel(self):
        """
        ฟังก์ชันแปลงไฟล์ JSON เป็น Excel
        - อ่านข้อมูลจากไฟล์ JSON
        - ใช้ฟังก์ชัน check_and_convert_json_value ในการแปลงข้อมูล
        - แสดงข้อความแจ้งเตือนเมื่อพบปัญหา
        - บันทึกข้อมูลที่ถูกแก้ไขแล้วลงในไฟล์ Excel
        """
        def log_to_file(msg):
            with open('log_process_converter.txt', 'a', encoding='utf-8') as logf:
                logf.write(msg + '\n')
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                for idx, row in enumerate(data):
                    try:
                        # แปลงแต่ละค่าโดยใช้ check_and_convert_json_value
                        row_dict = {}
                ...
                # กรณีข้อมูลเป็น dictionary เดี่ยว
                row_dict = {}
    ...
    def populate_table_from_json(self, data):
        """
        ฟังก์ชันแสดงข้อมูล JSON ในตาราง
        - กำหนดหัวคอลัมน์จากข้อมูล
        - แสดงข้อมูลในแต่ละแถว
        - เปิดใช้งานปุ่มจัดการแถว
        """
    ...
    def show_selected_row_json(self):
        """
        ฟังก์ชันแสดง JSON ของแถวที่เลือก
        - แปลงข้อมูลแถวเป็น JSON
        - แสดงในตัวแก้ไข JSON
        - เปิดใช้งานปุ่มอัปเดต
        """
        try:
    ...
    def update_row_from_json(self):
        """
        ฟังก์ชันอัปเดตแถวจาก JSON
        - อ่านข้อมูลจากตัวแก้ไข JSON
        - อัปเดตข้อมูลในตาราง
        - แสดงข้อความสถานะ
        """
        row_idx = self.table_widget.currentRow()
    ...
    def add_row(self):
        ...
        - เพิ่มแถวที่ตำแหน่งปัจจุบัน
        - บันทึกสถานะสำหรับการย้อนกลับ
        - อัปเดตข้อมูล JSON
        """
        try:
    ...
    def add_multiple_rows(self):
        """
        ฟังก์ชันเพิ่มหลายแถว
        - รับจำนวนแถวที่ต้องการเพิ่ม
        - เพิ่มแถวที่ตำแหน่งปัจจุบัน
        - บันทึกสถานะสำหรับการย้อนกลับ
        - อัปเดตข้อมูล JSON
        """
    ...
    def save_state(self):
        ...
        - บันทึกข้อมูลในตาราง
        - จัดการประวัติการย้อนกลับ/ทำซ้ำ
        - อัปเดตสถานะปุ่ม
        """
    ...
    def undo_action(self):
        """
        ฟังก์ชันย้อนกลับการกระทำ
        - เรียกคืนสถานะก่อนหน้า
        - อัปเดตสถานะปุ่ม
        - แสดงข้อความสถานะ
        """
