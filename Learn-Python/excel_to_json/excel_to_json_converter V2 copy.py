...
class MainWindow(QMainWindow):
    ...
    def save_json(self):
        ...
        for row_idx in range(self.table_widget.rowCount()):
            ...
            for col_idx, key in enumerate(headers):
                ...
                if item:
                    value = item.text()
                    try:
                        # Use check_and_convert_json_value function
                        converted_value = self.check_and_convert_json_value(value)
                        row_dict[key] = converted_value
                        
                        # If the value was fixed, add a message
                        if value != str(converted_value):
                            row_fixed.append(f"Column '{key}': Fixed format from '{value}' to '{converted_value}'")
                    except ValueError as e:
                        row_valid = False
        ...
        # Show preview
        preview_text.setPlainText(json.dumps(new_data, ensure_ascii=False, indent=4))
